...
export default function SkillTreeVisualization({
  ...
  const getStatusColor = (status: SkillProgressStatus): string => {
    switch (status) {
      ...
      case 'completed':
        return 'text-green-500 dark:text-green-400 border-green-500 bg-green-50 dark:bg-green-950/30';
      case 'in_progress':
        return 'text-blue-500 dark:text-blue-400 border-blue-500 bg-blue-50 dark:bg-blue-950/30';
      case 'available':
        return 'text-indigo-500 border-indigo-500 bg-indigo-50';
      case 'locked':
      default:
        return 'text-muted-foreground border-input bg-muted';
    }
  };
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border p-6">
        ...
        <div className="h-96 flex items-center justify-center bg-muted rounded-lg">
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground">Chargement de l&apos;arbre de compétences...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border p-6">
        ...
        <div className="h-96 flex items-center justify-center bg-muted rounded-lg">
          <div className="text-center">
            ...
            <p className="text-muted-foreground font-medium">Aucune compétence disponible</p>
            <p className="text-sm text-muted-foreground mt-1">L&apos;arbre de compétences sera bientôt disponible</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4 text-xs">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-muted-foreground">Maîtrisé</span>
          </div>
        </div>
      </div>
                ...
                >
                  ...
                  <foreignObject width={nodeWidth} height={nodeHeight}>
                    <div className="h-full flex flex-col items-center justify-center p-2 text-center">
                      {/* Icon */}
                      <div className={`mb-2 ${status === 'locked' ? 'text-muted-foreground' : 'text-current'}`}>
                        {node.icon_emoji ? (
            ...
            >
              ...
              <div className="sticky top-0 bg-card border-b border-border p-6 flex items-start justify-between">
                <div className="flex items-start gap-4 flex-1">
                  ...
                  <div className="flex-1">
                    <h3 className="text-xl font-bold text-foreground">{selectedNode.name_fr}</h3>
                    <p className="text-sm text-muted-foreground mt-1">{selectedNode.name}</p>
                    <div className="flex items-center gap-2 mt-2">
                      {selectedNode.difficulty && (
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                          selectedNode.difficulty === 'beginner' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' :
                          selectedNode.difficulty === 'intermediate' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800' :
                      ...
                      {selectedNode.estimated_time && (
                        <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-800">
              ...
              <div className="p-6 space-y-6">
                ...
                {selectedNode.detailed_explanation && (
                  <div>
                    <h4 className="text-sm font-semibold text-foreground mb-2">Explication détaillée</h4>
                    <p className="text-sm text-muted-foreground">{selectedNode.detailed_explanation}</p>
                  </div>
                )}
                        ...
                        return prereqNode ? (
                          <div
                            ...
                            className={`flex items-center gap-2 p-2 rounded-lg border ${
                              isCompleted ? 'border-green-200 bg-green-50 dark:bg-green-950/30' : 'border-border bg-muted'
                            }`}
                          >
                            ...
                            <span className={`text-sm ${isCompleted ? 'text-green-900' : 'text-muted-foreground'}`}>
                              {prereqNode.name_fr}
                            </span>
                          </div>
                        ) : null;
                    ...
                    <div className="space-y-3">
                      {selectedNode.examples.map((example, index) => (
                        <div key={index} className="border border-border rounded-lg p-3 bg-muted">
                          ...
                          <code className="text-xs text-foreground block bg-card p-2 rounded border border-border whitespace-pre-wrap">
                            {example.prompt}
                          </code>
                          {example.explanation && (
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="bg-green-50 dark:bg-green-950/30 rounded-lg p-3">
                        <div className="text-xs text-green-600 dark:text-green-400 font-medium mb-1">Pratiques réussies</div>
